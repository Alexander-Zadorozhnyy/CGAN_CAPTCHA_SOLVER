...
def main():
    path = f"trained_models/{CNN_MODEL}"
    cnn = load_model(path)

    path = os.path.join(os.getcwd(), f"../GAN/trained_models/{CGAN_MODEL}/Generator/generator.h5")
    gen = load_model(path)
    ...
    start_time = time.time()
    test_not_trained_cap(cnn, path=os.path.join(os.getcwd(), 'generated_data'))
